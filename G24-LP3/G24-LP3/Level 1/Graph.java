...
class Graph implements Iterable<Vertex> {
    ...
    public boolean negative_G=false;
    public boolean uniform_G=true;
    ...
    public void check_weight(Graph G){
     //To get the weight of first edge in the graph
        int w=0;
     for(Vertex v:G){
        for(Edge e: v.Adj){
            w=e.Weight;
     ...
     //check the whole graph for negative weight
     for(Vertex v:G){
        for(Edge e: v.Adj){
             if(e.Weight<0){
             G.negative_G=true;
     ...
     public boolean DAC_check(Graph G){ 
         boolean bol=false;
            int count=0;
          for(Vertex u: G){
            u.seen = false;
